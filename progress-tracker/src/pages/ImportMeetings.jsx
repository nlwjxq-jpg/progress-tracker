...
export default function ImportMeetings() {
  ...
  const handleAiParse = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${baseUrl}/functions/v1/ai-parse-meetings`, {
        method: 'POST',
      ...
      const meetings = (result.meetings || []).map((m, i) => ({
        ...m, _id: i, status: m.status || 'pending'
      }))
      setParsedMeetings(meetings)
      setSelectedMeetings(new Set(meetings.map(m => m._id)))
      setStep('preview')
    } catch (err) {
  ...
  const toggleMeeting = (id) => {
    ...
    setSelectedMeetings(next)
  }

  const toggleAll = () => {
    selectedMeetings.size === parsedMeetings.length
      ? setSelectedMeetings(new Set())
      : setSelectedMeetings(new Set(parsedMeetings.map(m => m._id)))
  }

  const handleImport = async () => {
    const toImport = parsedMeetings.filter(m => selectedMeetings.has(m._id))
    if (toImport.length === 0) return
    setImporting(true)
    setError('')
    try {
      const { data: existing } = await supabase.from("meetings").select('meeting_name')
      const existingNames = new Set((existing || []).map(m => m.meeting_name.trim()))
      let added = 0, skipped = 0
      const now = new Date().toISOString()
      for (const meeting of toImport) {
        if (existingNames.has(meeting.meeting_name.trim())) { skipped++; continue }
        const { error: insertErr } = await supabase.from("meetings").insert({
          meeting_name: meeting.meeting_name.trim(),
        ...
        if (!insertErr) { added++; existingNames.add(meeting.meeting_name.trim()) }
      }
  ...
  const reset = () => {
    setFile(null); setTextContent(''); setParsedMeetings([])
    setSelectedMeetings(new Set()); setStep('upload')
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        <h2 className="text-2xl font-bold text-gray-800">导入会议任务表</h2>
      </div>

      {step === 'upload' && (
        <div className="card space-y-4">
          <p className="text-sm text-gray-500">上传 Excel (.xlsx) 或 CSV 会议任务表，AI 将自动识别会议名称、时间、部门任务和责任人。</p>
      ...
      {step === 'preview' && (
        <div className="card space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">解析结果（{parsedMeetings.length} 项会议任务）</h3>
            <div className="flex gap-2">
              <button onClick={toggleAll} className="text-sm text-blue-600 hover:underline">
                {selectedMeetings.size === parsedMeetings.length ? '取消全选' : '全选'}
              </button>
              <span className="text-sm text-gray-400">已选 {selectedMeetings.size}</span>
            </div>
          </div>

          <div className="max-h-96 overflow-y-auto space-y-2">
            {parsedMeetings.map(m => (
              <div key={m._id} onClick={() => toggleMeeting(m._id)}
                className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${selectedMeetings.has(m._id) ? 'border-blue-300 bg-blue-50' : 'border-gray-200 bg-white'}`}>
                <div className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${selectedMeetings.has(m._id) ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
                  {selectedMeetings.has(m._id) && <CheckCircle size={12} className="text-white" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm">{m.meeting_name}</p>
                  {m.task_description && <p className="text-xs text-gray-400 mt-0.5">{m.task_description}</p>}
                  <div className="flex gap-3 mt-1 text-xs text-gray-500">
                    {m.meeting_date && <span>📅 {m.meeting_date}</span>}
                    {m.assignee && <span>👤 {m.assignee}</span>}
                    <span className={`px-1.5 py-0.5 rounded ${m.status === 'completed' ? 'bg-green-100 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
                      {m.status === 'completed' ? '已完成' : '待开始'}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="flex gap-3">
            <button onClick={handleImport} disabled={importing || selectedMeetings.size === 0} className="btn-primary flex items-center gap-2">
              {importing ? '导入中...' : <><CheckCircle size={16} /> 导入选中的 {selectedMeetings.size} 项会议任务</>}
            </button>
        ...
        <div className="card space-y-4 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              新增 <span className="text-green-600 font-bold">{importResult.added}</span> 项会议任务，
              跳过 <span className="text-yellow-600 font-bold">{importResult.skipped}</span> 项重复
            </p>
          </div>
          <div className="flex gap-3 justify-center">
            <button onClick={() => navigate('/meetings')} className="btn-primary">查看会议列表</button>
